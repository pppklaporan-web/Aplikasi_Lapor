...
listWrap.innerHTML = items.map(i => `
  <div class="card-report">
    ...
    <div class="meta">ID: ${escapeHtml(i.id)} · ${escapeHtml(i.waktu)}</div>

    <div style="margin-top:8px">
      <label>Status
        <select data-id="${escapeHtml(i.id)}" class="statusSel">
          <option value="Masuk" ${i.status === 'Masuk' ? 'selected' : ''}>Masuk</option>
          <option value="Proses" ${i.status === 'Proses' ? 'selected' : ''}>Proses</option>
          <option value="Selesai" ${i.status === 'Selesai' ? 'selected' : ''}>Selesai</option>
        </select>
      </label>

      <label style="margin-top:8px">Petugas
        <input type="text" data-id="${escapeHtml(i.id)}" class="petugasInput"
          placeholder="Nama petugas" value="${escapeHtml(i.petugas || '')}" />
      </label>

      <div style="margin-top:8px">
        <button class="updateBtn small" data-id="${escapeHtml(i.id)}">Simpan</button>
      </div>
    </div>
  </div>
`).join('');
